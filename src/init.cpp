...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to Eved / RPC client
            std::string strUsage = _("Eve version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  eved [options]                     " + "\n" +
                  "  eved [options] <command> [params]  " + _("Send command to -server or eved") + "\n" +
                  "  eved [options] help                " + _("List commands") + "\n" +
                  "  eved [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "eve:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: eve.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: eved.pid)") + "\n" +
        ...
        "\n" + _("SSL options: (see the Eve Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Eve is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Eve version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "Eve server starting\n");
   ...
   //     return InitError(_("You need to rebuild the databases using -reindex to change -txindex"));
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Eve") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Eve to complete") << "\n";
