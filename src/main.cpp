...
const string strMessageMagic = "Eve Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 64 * COIN;
        ...
        if(nHeight == 8)
     {
        nSubsidy = 2600000 * COIN;           //approx Five percent of total coins goes to team  these are to aid with costings to advance E-ledger

     }
        if(nHeight == 129)
     {
        nSubsidy = 6020616 * COIN;        //As of block 195292 total supply minus dec burn addy 1111111111111111111114oLvT2 on Acp partner network. Swapable network hop plus proof of burn on Acp network
	 }
		if(nHeight == 3000)
     {
        nSubsidy = 10000 * COIN;        // block 3000 Miners bonus block network distro increased.
	 }	
	    if(nHeight == 4500)
     {
        nSubsidy = 12000 * COIN;        // block 4500 Miners bonus block network distro increased.
	 }	
		if(nHeight == 21000)
     {
        nSubsidy = 15000 * COIN;        // block 21000 Miners bonus block network distro increased.
	 }
...
static const int64 nBlockDigishieldStart = 2000;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "bitcoinx2 segwit bcc 01/Aug/2017 bitcoin transactions frozen on exchanges";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EveMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EveMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { while (true) {
        ...
        printf("Running EveMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
